...
From compcerttso.clightTSO Require Import Csyntax.
Module Clight.
...
Definition env := PTree.t (block * type). (* map variable -> location *)
...
Inductive is_false: val -> type -> Prop :=
  | is_false_int: forall sz sg a,
      is_false (Vint Int.zero) (Tint sz sg a)
  | is_false_pointer: forall t a,
      is_false (Vint Int.zero) (Tpointer t a)
 | is_false_float: forall f sz a,
      Float.cmp Ceq f Float.zero ->
      is_false (Vfloat f) (Tfloat sz a).

Inductive is_true: val -> type -> Prop :=
  | is_true_int_int: forall n sz sg a,
      n <> Int.zero ->
      is_true (Vint n) (Tint sz sg a)
  | is_true_pointer_int: forall b ofs sz sg a,
      is_true (Vptr b ofs) (Tint sz sg a)
  | is_true_int_pointer: forall n t a,
      n <> Int.zero ->
      is_true (Vint n) (Tpointer t a)
  | is_true_pointer_pointer: forall b ofs t a,
      is_true (Vptr b ofs) (Tpointer t a)
 | is_true_float: forall f sz a,
      Float.cmp Cne f Float.zero ->
      is_true (Vfloat f) (Tfloat sz a).
...
Proof.
  ...
  try (destruct (Float.cmp Cne f Float.zero) as [] eqn: X);
...
Proof.
  intros. destruct v; destruct ty;
  try (destruct (Int.eq_dec i Int.zero); subst);
  try (destruct (Float.cmp Ceq f Float.zero) as [] eqn: X);
  try (by right; intro H; inv H; try rewrite X in *; auto);
  try (by left; constructor; auto).
Defined.
...
Definition blocks_of_env (e:env) : list (block * type) :=
   (List.map (@snd ident (block * type)) (PTree.elements e)).

Definition sorted_pointers_of_env (e:env) : list (block * type) :=
 let (l,_) := mergesort _ 
...
Proof.
  ...
  destruct (eval_of_val v) as [ev|] eqn : Ev; simpl; [|done].
  intro E; inv E. simpl. 
...
Ltac asm_match_destruction := match goal with
    ...
    | H : context[match ?v with (_,_) => _ end] |- _ => destruct v as [] eqn: ?
    ...
    | H : context[if Val.has_type ?E ?F then _ else _] |- _ => destruct (Val.has_type E F) as [] eqn: ?
    | H : context[if valid_arg ?a ?b ?c ?d then _ else _] |- _ => destruct (valid_arg a b c d) as [] eqn: ?; try discriminate
    | H : context[if ?v then _ else _] |- _ => destruct v; try discriminate
    | H : context[(match ?v with
          | Vundef => _ | Vint _ => _ | Vfloat _ => _ | Vptr _ => _ end)] |- _ => destruct v as [] eqn: ?
    | H : context[(match ?v with
          | Some _ => _ | None => _ end)] |- _ => destruct v as [] eqn: ?
    | H : context[(match ?v with
          | OK _ => _ | Error _ => _ end)] |- _ => destruct v as [] eqn: ?
    | H : context[(match ?v with
          | Internal _ => _ | External _ _ _ => _ end)] |- _ => destruct v as [] eqn: ?
    | H : context[(match ?v with
          | nil => _ | _::_ => _ end)] |- _ => destruct v as [] eqn: ?
    | H : context[(match ?v with
          | Tvoid => _ | Tint _ _ => _ | Tfloat _ => _ | Tpointer _ => _
          | Tarray _ _ => _ | Tfunction _ _ => _ | Tstruct _ _ => _
          | Tunion _ _ => _ | Tcomp_ptr _ => _ end)] |- _ => destruct v as [] eqn: ? 
    | H : context[(match ?v with
        | TEext _ => _ | TEmem _ => _ | 
              TEtau
               => _ | TEexit => _ 
    ...
    | H : context[(match ?v with 
           ...
           | By_nothing => _ end)] |- _ => destruct v as [] eqn: ?
...
Proof. 
  ...
  destruct s as [[[]] | [[]]|vv ? []|[]| | | | |]; simpl in H;
   clarify;
   try (destruct (cast_ex vv t t0) as [] eqn : EQ; try apply cast_ex_some in EQ; clarify);
...
Ltac goal_match_destruction := match goal with
    ...
    | |- context[(match ?v with
          | Vundef => _ | Vint _ => _ | Vfloat _ => _ | Vptr _ => _ end)] => destruct v as [] eqn: ?
    | |- context[(match ?v with
          | Some _ => _ | None => _ end)] => destruct v as [] eqn: ?
    | |- context[(match ?v with
          | MObjStack => _ | MObjHeap => _ | MObjGlobal => _ end)] => 
            destruct v as [] eqn: ?
    | |- context[(match ?v with
          | Internal _ => _ | External _ _ _ => _ end)] => destruct v as [] eqn: ?
    | |- context[(match ?v with
          | nil => _ | _::_ => _ end)] => destruct v as [] eqn: ?
    | |- context[(match ?v with
          | Tvoid => _ | Tint _ _ => _ | Tfloat _ => _ | Tpointer _ => _
          | Tarray _ _ => _ | Tfunction _ _ => _ | Tstruct _ _ => _
          | Tunion _ _ => _ | Tcomp_ptr _ => _ end)] => destruct v as [] eqn: ? 
    | |- context[(match ?v with
        | TEext _ => _ | TEmem _ => _ | 
              TEtau
             => _ | TEexit => _ 

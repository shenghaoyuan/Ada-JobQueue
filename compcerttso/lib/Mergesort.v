...
From Coq Require Import List Permutation Program.Wf Lia.
...
Next Obligation.
  ...
  split. lia.
  intro H0.
  destruct ll; simpl in H0. elimtype False; lia. 
  destruct ll; simpl in H0. elimtype False; lia. 
  elim (n l l0 ll). auto.
Qed.
...
Qed.
Next Obligation.
  destruct (merge_pairs ll
             (fun (l : list A) (H0 : In l ll) =>
              eq_ind_r
                (fun wildcard' : list (list A) =>
                 (forall l0 : list A, l0 :: nil <> wildcard') ->
                 nil <> wildcard' -> Sorted l)
                (fun (_ : forall l0 : list A, l0 :: nil <> ll) (_ : nil <> ll) =>
                 SRT l H0) eq_refl n n0))
                   ...
                   (proj1_sig
                      (merge_pairs ll
                         (fun (l0 : list A) (H0 : In l0 ll) =>
                          eq_ind_r
                            (fun wildcard' : list (list A) =>
                             (forall l1 : list A, l1 :: nil <> wildcard') ->
                             nil <> wildcard' -> Sorted l0)
                            (fun (_ : forall l1 : list A, l1 :: nil <> ll)
                               (_ : nil <> ll) => SRT l0 H0) eq_refl n n0)))) =>
         merge_iter_func_obligation_4 ll SRT ll (conj n n0) eq_refl l H0)
        (merge_iter_func_obligation_5 ll SRT merge_iter ll (conj n n0) eq_refl)) as [mll (S & P & ST)].
  simpl in *.
  split. auto.
  destruct (merge_pairs ll
                (fun (l : list A) (H0 : In l ll) =>

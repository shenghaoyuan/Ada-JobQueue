...
From compcert.common Require Import AST Values Events Memory Globalenvs.

From compcerttso.common Require Import AST Values Events Simulations Libtactics.
From compcerttso.lib Require Import Coqlib Maps.
...
Structure SEM := mkSEM 
  {
    ...
    SEM_find_symbol : SEM_GE -> ident -> option val ;
    SEM_step : SEM_GE -> SEM_ST -> thread_event -> SEM_ST -> Prop ;
    SEM_init : SEM_GE -> val -> list val -> option SEM_ST ;
    SEM_progress_dec :
...
Inductive mm_arglist (tso: mm_state) (t: thread_id) :
    list (val * memory_chunk + val) -> list val -> mm_state -> Prop :=
| mm_arglist_refl: 
    mm_arglist tso t nil nil tso
| mm_arglist_val:  forall locs vals tso' v
    (MA: mm_arglist tso t locs vals tso'),
    mm_arglist tso t (inr (val * memory_chunk) v :: locs) (v :: vals) tso'
...
Inductive mm_ext_arglist (tso: mm_state) (t: thread_id) :
    list (val * memory_chunk + eventval) -> list eventval -> mm_state -> Prop :=
| mm_ext_arglist_refl:
    mm_ext_arglist tso t nil nil tso
| mm_ext_arglist_val:  forall locs vals tso' v
    (MA: mm_ext_arglist tso t locs vals tso'),
    mm_ext_arglist tso t (inr (val * memory_chunk) v :: locs) (v :: vals) tso'
...
Inductive mm_arglist_fail (tso: mm_state) (t: thread_id) :
    list (val * memory_chunk + val) -> Prop :=
...
| mm_arglist_fail_val:  forall locs v
    ...
    mm_arglist_fail tso t (inr (val * memory_chunk) v :: locs)
...
Inductive mm_ext_arglist_fail (tso: mm_state) (t: thread_id) :
    list (val * memory_chunk + eventval) -> Prop :=
...
| mm_ext_arglist_fail_val:  forall locs v
    ...
    mm_ext_arglist_fail tso t (inr (val * memory_chunk) v :: locs)
...
Inductive step : state -> fullevent -> state -> Prop :=
  ...
  | step_extcallmem_fail:
    ...
    step (tso, st) (Evisible Efail) (tso, st')
  | step_startmem:
    forall s s' args tid tso tso' tso'' st st' v sinit newtid fn ptr
      (tidSTEP: Sem_step ge s (TEstartmem fn args) s')
      (tsoSTEP: mm_arglist tso tid (fn::args) (ptr::v) tso')
      (tsoSTEP2: mm_step tso' (MMstart tid newtid) tso'')
      (Gtid: st ! tid = Some s)
      (Gnewtid: st ! newtid = None)
      (EQst': st' = (st ! tid <- s') ! newtid <- sinit)
      (INIT: Sem.(SEM_init) ge ptr v = Some sinit),
    step (tso, st) Etau (tso'', st')
  ...
  | step_startmem_spawn_fail:
    forall s s' tid tso st st' lfn largs fn args tso'
      ...
      (Gtid: st ! tid = Some s)
      (INIT: match fn with Vptr _ _ => Sem.(SEM_init) ge fn args = None 
                         | _ => True end)
      (EQst': st' = (st ! tid <- s')),
    step (tso, st) (Evisible Efail) (tso, st').
...
Hint Resolve MM_inv_init MM_inv_step : ld.
...
Proof.
  by intros; apply PTree.extensionality; intros; rewrite PTree.gmap, !PTree.gempty. 
Qed.
...
Proof.
  intros; apply PTree.extensionality; intro x. 
...
Proof.
  intros; apply PTree.extensionality; intros; rewrite PTree.gsspec.
...
Lemma init_consistent:
  forall {prg args ge t} (INIT: init prg args ge t),
    consistent t.
Proof. destruct 1; des; clarify; red; unfold fst, snd; rewrite maps, mapempty; auto with ld. Qed.
...
Proof.
  red; intros; red in CONS; inv STEP; simpl in *; eauto; rewrite ?maps; simpl;
    ...
    - eapply MM_inv_step; eauto; instantiate; simpl.
      ...
      apply PTree.extensionality; intros; rewrite PTree.gmap, !PTree.grspec, PTree.gmap.
...
Hint Resolve step_preserves_consistency : ld.
...
Lemma taustar_preserves_consistency:
  forall s s',
    ...
    consistent s ->
    consistent s'.
Proof. induction 1; eauto with ld. Qed.

Hint Resolve taustar_preserves_consistency : ld.
...
Lemma taustep_preserves_consistency:
  forall s e s',
    ...
    consistent s ->
    consistent s'.
Proof. destruct 1; des; eauto with ld. Qed.
...
Proof. destruct 1; des; eauto with ld. Qed.
  ...
  Lemma ptr_or_not:
    forall v, {match v with Vptr _ _ => True | _ => False end} + {match v with Vptr _ _ => False | _ => True end}.
...
*)
  ...
  (* The following section is just temporary, until 
    ...
    Proof.
      ...
      exists w. 
      destruct (PTree.get w m) as [e|] eqn : E; try done.
...
Lemma stuck_imp_no_threads:
  forall s
    ...
    snd s = PTree.empty _.
Proof.
  intros; apply PTree.extensionality; intro t; rewrite PTree.gempty.
  destruct ((snd s) ! t) as [ts|] eqn: X; clarify.
  destruct s as [s thr].
  ...
  Case "TEstart".
    ...
    destruct (SEM_init _ ge sp sa) as [ns|] eqn : INI.
        (* Everything OK - spawn *)
